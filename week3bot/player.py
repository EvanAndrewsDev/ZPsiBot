...
pre_flop_fold = []
def fold_pre_flop(hand):
    if hand[0].suit != hand[1].suit and hand[0].rank != hand[1].rank and (hand[0].rank + hand[1].rank < 10):
        print("folded pre-flop since the sum was", hand[0].rank + hand[1].rank)
        return True
    return False
...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if BidAction in legal_actions:     #Simply pay the most for the bid due to GT
            return BidAction(my_stack)
        
        #preflop logic
        if street == 0:
            if fold_pre_flop(hand):
                return bail_option(legal_actions)
        ...
        if equity < 0.5:
            ...
            if equity <= 0.6:
                if random.random() < 0.2:
                    if RaiseAction in legal_actions:
                        return RaiseAction(min(int(0.3 * pot), max_raise))
                    return CallAction()
                ...
                return CallAction()
            if RaiseAction in legal_actions:
                return RaiseAction(min(int(0.5 * pot), max_raise))
